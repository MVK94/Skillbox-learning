...
то есть две команды, по 20 участников в каждом. В них хранятся очки каждого участника —
...
(от 5 до 10). Для этого найдите подходящую функцию из модуля random. Затем сгенерируйте третий список,
...
double_command =[round(random.uniform(5, 10), 2) for j in range(20)]

print(double_command)

winner = [first_command[n] if first_command[n] > double_command[n] else double_command[n] for n in range(20) ]
print(winner)"""

import random
def generate_team(n):
    return [round(random.uniform(5, 10), 2) for i in range(n)]


first_team = generate_team(20)
second_team = generate_team(20)

winner = [a if a > b else b for a, b in zip(first_team, second_team)]
